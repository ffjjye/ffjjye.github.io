...
import { mockUpdates, mockReviews } from "@/mock/index.js";
...
export default {
  ...
  methods: {
    loadData() {
      // 直接使用 mock 数据
      this.blogDT = mockUpdates.map(update => ({
        blogID: update.id,
        ...
        authorImage: require('@/assets/avatar.jpg')
      }));

      this.bookReviewDT = mockReviews.map(review => ({
